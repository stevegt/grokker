...
func sendQueryToLLM(query string, llm string, selection, backgroundContext string, inputFiles []string, outFiles []string, wordCount int) string {
	...
	sysmsg = fmt.Sprintf("%s\n\nYou MUST limit the discussion portion of your response to no more than %d words.  Output files (marked with ---FILE-START and ---FILE-END blocks) are not counted against this limit and can be unlimited size. You MUST ignore any previous instruction regarding a 10,000 word goal.", sysmsg, wordCount)
	...
	// repeat until we get a valid response that is less than wordCount limit
	...
	for {
		...
		// Convert token count to approximate word count (rough estimate: 1.3 tokens per word)
		approxWords := count / 4 * 3
		if approxWords > wordCount {
			log.Printf("Response exceeds word count limit:\n\n%s", discussionOnly)
			sysmsg += fmt.Sprintf("\n\nYour previous response was approximately %d words, which exceeds the limit of %d words.  You ABSOLUTELY MUST provide a more concise answer that fits within the limit.", approxWords, wordCount)
			prompt += fmt.Sprintf("\n\nYou MUST provide a more concise answer that fits within the %d word limit.", wordCount)
			log.Printf("Response word count approximately %d exceeds limit of %d; retrying...", approxWords, wordCount)
