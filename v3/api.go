...
func (g *GrokkerInternal) Chat(sysmsg, prompt, fileName string, limitContext, fast bool) (resp string, err error) {
	defer Return(&err)
	...
	// get response
	resp, _, err = history.continueChat(prompt, limitContext, fast)
	Ck(err)
	return
}
