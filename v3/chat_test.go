...
func TestChatSummarization(t *testing.T) {
	...
	res, err := grok.Chat("", "Pretend a blue widget has a red center.", "chat1", ContextAll, nil, nil, 0, 0)
	...
	res, err = grok.Chat("", "What color is the center of the blue widget?", "chat1", ContextAll, nil, nil, 0, 0)
	match = cimatch(res, "red")
	Tassert(t, match, "CLI did not return expected output: %s", res)
