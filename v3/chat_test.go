...
func TestChatSummarization(t *testing.T) {
	...
	res, debug, err := history.continueChat("Talk about complex systems.", ContextAll, nil, nil)
	...
	for i := 0; i < 3; i++ {
		...
		res, debug, err = history.continueChat(prompt, ContextAll, nil, nil)
		Tassert(t, err == nil, "error continuing chat: %v", err)
		err = history.Save()
		Ck(err)
	...
	for i := 0; i < 10; i++ {
		if true {
			...
			res, _, err = history.continueChat(prompt, ContextAll, nil, nil)
			Tassert(t, err == nil, "error continuing chat: %v", err)
			err = history.Save()
			Ck(err)
			// ensure the response contains the expected output
